...
class PatientInfoV(db.Model):
    ...
    def to_dict(self):
        my_dict = {
            'id': self.id, 'name': self.name.replace('/','').replace('\\',''),
...
class SampleInfoV(db.Model):
    ...
    send_t = db.Column(db.String(100))  # 送检日期
    detect_t = db.Column(db.String(100))  # 检测日期
    ...
    def to_dict(self):
        my_dict = {
            ...
            'pnumber': self.pnumber, 'receive_t': time_set(self.receive_t), 'detect_t': time_set(self.detect_t),
