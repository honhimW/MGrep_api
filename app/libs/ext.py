...
def ssh_con():
    ...
    ssh.connect('192.168.1.108', 22, 'honhim', 'honhim1206')
    return ssh


def scp_to_108(ssh, indir, remote_dir):
    with SCPClient(ssh.get_transport())as scp:
        scp.put(indir, recursive=True, remote_path=remote_dir)
...
def get_sample(applys):
    ...
    for apply in applys:
        ...
        if not (apply.pathological_date is None):
            apply.pathological_date = apply.pathological_date.strftime('%Y-%m-%d')
